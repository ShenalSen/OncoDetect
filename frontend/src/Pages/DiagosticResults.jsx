        ...
        <div className="ml-5 p-10 pt-10 bg-gray-100 min-h-screen">
            ...
            <div className="max-w-4xl bg-white p-6 rounded-lg shadow-lg mx-auto">
                <div className="text-center mb-6">
                    <h2 className="text-xl font-semibold">Mammogram Image</h2>
                    <img src={cancerImage} alt="Mammogram" className="w-3/4 mx-auto rounded-lg border border-gray-300 my-4" />
                    <p className="text-gray-600">Detected anomalies (if any) are highlighted.</p>
                </div>
                ...
                {results && (
                    <div className="bg-gray-50 p-4 rounded-lg shadow-md mb-6">
                        <h2 className="text-lg font-semibold text-center mb-4">Results Summary</h2>
                        <ul className="text-gray-700 space-y-2">
                            <li><strong>Patient Name:</strong> {results.patient_name || "Unknown"}</li>
                            <li><strong>Diagnosis:</strong> 
                                <span className={`font-semibold ${results.final_result === "Malignant" ? "text-red-500" : "text-green-500"}`}>
                                    {results.final_result || "Pending"}
                                </span>
                            </li>
                            <li><strong>Confidence Level:</strong> 
                                <span className="text-green-500 font-semibold">{results.confidence_level || "N/A"}%</span>
                            </li>
                            <li><strong>Doctor Recommendation:</strong> 
                                <span className="text-blue-500 underline cursor-pointer">{results.doctor_recommendation || "Not Available"}</span>
                            </li>
                        </ul>
                    </div>
                )}
                ...
                <div className="bg-gray-50 p-4 rounded-lg shadow-md mb-6">
                    <h2 className="text-lg font-semibold mb-4">Patient Information</h2>
                    <p><strong>Age:</strong> {results.age || "Unknown"}</p>
                    <p><strong>Sex:</strong> {results.sex || "Unknown"}</p>
                    <p><strong>Medical History:</strong> <span className="text-blue-500 font-semibold">{results.medical_history || "No significant history"}</span></p>
                </div>

                <div className="bg-white p-4 rounded-lg shadow-md mb-6">
                    <h2 className="text-lg font-semibold mb-4">Additional Insights</h2>
                    <p><strong className="text-red-500">Note:</strong> {results.additional_insights || "Further tests are recommended."}</p>
                </div>
